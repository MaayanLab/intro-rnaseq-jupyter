...
run_characteristic_direction <- function(rawcount_dataframe, design_dataframe, constant_threshold=1e-5) {

	# Log-transform
	log_rawcount_dataframe <- log10(rawcount_dataframe + 1)

	# Get dataframes
	experimental_dataframe <- log_rawcount_dataframe[,rownames(design_dataframe)[design_dataframe[,'experimental'] == 1]]
	control_dataframe <- log_rawcount_dataframe[,rownames(design_dataframe)[design_dataframe[,'control'] == 1]]
...
run_limma <- function(rawcount_dataframe, design_dataframe, adjust="BH") {
