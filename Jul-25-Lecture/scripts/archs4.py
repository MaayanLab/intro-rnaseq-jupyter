...
def fetch_dataset(dataset_accession):
	...
	if platform_accession in ['GPL11154', 'GPL13112', 'GPL16791', 'GPL17021']:
		...
		metadata_dataframe = pd.DataFrame([x.split('\t') for x in data.split('\n')[1:] if any(y in x for y in ['!Sample_geo_accession', '!Sample_title', '!Sample_characteristics_ch1'])]).set_index(0)

		# Get title conversion
		sample_title_dict = {sample_accession: '{sample_title} ({sample_accession})'.format(**locals()) for sample_accession, sample_title in metadata_dataframe.loc[['!Sample_geo_accession', '!Sample_title']].T.as_matrix() if sample_accession}
		...
		# Create dict
		# sample_metadata_dataframe = pd.DataFrame({sample_accession: metadata_dict for sample_accession, metadata_dict in zip(metadata_dataframe.loc['!Sample_geo_accession'], metadata_dict)}).T

		# Return
		# return rawcount_dataframe, sample_metadata_dataframe, sample_title_dict
		return rawcount_dataframe, sample_title_dict
